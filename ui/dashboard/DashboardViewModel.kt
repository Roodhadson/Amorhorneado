...
class DashboardViewModel(private val repository: IngredientRepository) : ViewModel() {
    ...
    // Mock Weather State
    private val _weatherState = MutableStateFlow(WeatherInfo("28°C", "Sunny"))
    ...
    val dashboardUiState: StateFlow<DashboardUiState> = combine(
        ...
        for (i in 6 downTo 0) {
            ...
            val totalForDay = sales.filter { it.date in startOfDay..endOfDay }
        ...
        // Previous Week (simplified for comparison)
        ...
        // 2. Monthly Sales
        val calMonth = Calendar.getInstance().apply { set(Calendar.DAY_OF_MONTH, 1); set(Calendar.HOUR_OF_DAY, 0) }
        ...
        val monthlyPercent = 0.0 // Simplified

        // 3. Top Product Calculation
        ...
        // 4. Sales History for Top Product
        val topProductSales = if (topProduct != null) {
            sales.filter { it.recipeId == topProduct.id || (it.recipeId == -1 && it.recipeTitle.contains(topProduct.title, ignoreCase = true)) }
    ...
    fun updateUserConfig(name: String, imagePath: String?) {
        viewModelScope.launch { repository.insertUserConfig(UserConfig(name = name, profileImagePath = imagePath)) }
    }
...
data class WeatherInfo(val temp: String, val condition: String)
